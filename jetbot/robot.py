...
class Robot(SingletonConfigurable):
    ...
    # config
    left_motor_channel = traitlets.Integer(default_value=MotorDriver.MOTOR_A).tag(config=True)
    left_motor_alpha = traitlets.Float(default_value=1.0).tag(config=True)
    right_motor_channel = traitlets.Integer(default_value=MotorDriver.MOTOR_B).tag(config=True)
